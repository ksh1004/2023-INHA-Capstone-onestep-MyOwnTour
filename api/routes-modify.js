...
async function modifyRoute(req, res) {
  // 비동기적 동작
  ...
  try {
    ...
    const course_id = userData[0].courseId; // course_id 값
    const start_day = userData[0].startDay;
    const finish_day = userData[0].finishDay;
    ...
    for (let i = 0; i < userData.length; i++) {
      ...
      const user_info = {
        ...
        course_id: course_id,
        route_name: userData[i].children, // 장소 이름
    ...
    res.status(200).json({
      message: `user_id: ${userId} 의 모든 경로가 정상적으로 수정되었습니다.`,
    });
  } catch (error) {
    // 에러가 발생한 경우, 500 상태 코드와 함께 에러 메시지를 응답
