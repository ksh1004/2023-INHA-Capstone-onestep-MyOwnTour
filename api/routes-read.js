...
async function readRoute(req, res) {
  // 비동기적 동작
  ...
  try {
    ...
    if (routes.length === 0) {
      // 만약 결과가 없다면, 404 상태 코드와 함께 메시지를 응답
    ...
    for (let i = 0; i < routes.length; i++) {
      if (
        !course_id_data.find(
          (course_id_data) => course_id_data.course_id === routes[i].course_id
        )
      ) {
        course_id_data.push({
      ...
      for (let j = 0; j < routes.length; j++) {
        if (course_id_data[i].course_id === routes[j].course_id) {
          const user_info = {
            ...
            image_url: routes[j].route_imageUrl,
