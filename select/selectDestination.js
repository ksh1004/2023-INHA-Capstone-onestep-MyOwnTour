...
async function selectDestination(userId, course_id) {
  try {
    const user = await Personalities.findOne({
      user_id: userId,
      course_id: course_id,
    }); // 해당 userId의 personalities 데이터를 가져온다.
    if (!user) {
      // 유저가 존재하지 않는 경우
    ...
    // 시작 좌표 설정
    let startpoint_name = travel_destination + "고속버스터미널"; // 시작좌표 이름(지역 + 버스터미널)
    let startpoint_apiUrl =
      "https://maps.googleapis.com/maps/api/place/findplacefromtext/json"; // google api Url
    let startpoint_response = await axios.get(startpoint_apiUrl, {
      // api 호출
      params: {
    ...
    results.push({
      ...
      latitude: latitude,
      longitude: longitude,
      address: startpoint_address,
      type: "여행지",
      image_url: photo_url,
    });
    //console.log(results[0])
    const user_rankDestinationData = await setUserDestinationRank(
      userId,
      course_id
    ); // 여행지 추천 갯수를 가져오고, 저장하는 data
    const user_rankFoodData = await setUserFoodRank(userId, course_id); // 음식 추천 갯수를 가져오고, 저장하는 data
    const user_rankAccommodationData = await setUserAccommodationRank(
      userId,
      course_id
    ); // 숙소 추천 갯수를 가져오고, 저장하는 data
    // 여행지, 음식점, 숙소 순서 정렬
    ...
    const sortedAccommodationKeywords = Object.entries(
      ...
      while (true) {
        for (const [, value] of sortedDestinationKeywords) {
          ...
          for (const keyword of value.keywords) {
            ...
            let searchvalue = await searchKeyword(
              latitude,
              longitude,
              results,
              radius,
            ...
            if (searchvalue.length > 0 && searchvalue[0].result_value === 1) {
              // 여행지 값이 존재하면
              ...
              results.push({
                ...
                seq: seq_value,
                day: 1,
                name: searchvalue[0].name,
                rating: searchvalue[0].rating,
                latitude: searchvalue[0].latitude,
                longitude: searchvalue[0].longitude,
                address: searchvalue[0].address,
                type: "여행지",
                place_id: searchvalue[0].place_id,
                image_url: searchvalue[0].image_url,
              });
              ...
              while (!else_found) {
                ...
                searchvalue = await searchKeyword(
                  latitude,
                  longitude,
                  results,
                  radius,
        ...
        for (const [, value] of sortedDestinationKeywords) {
          ...
          if (seq_value === 9) {
            ...
            for (const keyword of value.keywords) {
              ...
              let searchvalue = await searchKeywordWithLocation(
                travel_destination,
                results,
                radius,
                keyword
              ...
              if (searchvalue.length > 0 && searchvalue[0].result_value === 1) {
                ...
                while (!else_found) {
                  // 여행지를 찾을 때까지 반복
                  radius = radius + 5000; // 반경 5km 증가
                  searchvalue = await searchKeywordWithLocation(
                    travel_destination,
                    results,
                    radius,
                    keyword
    ...
    if (data.results.length > 0) {
      for (const place of data.results) {
        ...
        if (
          ...
          !result.find(
            (result) =>
              result.place_id === place.place_id && result.name === place.name
          )
        ) {
          let imageUrl = "";
...
async function searchKeywordWithLocation(
  locationName,
  result,
  radius,
  keyword
  ...
  try {
    ...
    if (places.length > 0) {
      for (const place of places) {
        ...
        if (
          rating >= 1 &&
          !result.find((result) => result.place_id === place.place_id)
        ) {
          let photo_url = "";
          if (place.photos && place.photos.length > 0) {
